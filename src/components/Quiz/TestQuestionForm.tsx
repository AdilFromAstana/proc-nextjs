...
export default function TestQuestionForm({ onAdd, onCancel }: Props) {
  ...
  const toggleAnswerCorrect = (id: string) => {
    if (allowMultipleAnswers) {
      // Для множественного выбора можно отмечать несколько
      setAnswerOptions(
        answerOptions.map((option) =>
          option.id === id
      ...
      // Для одиночного выбора - только один правильный
      setAnswerOptions(
        answerOptions.map((option) => ({
          ...option,
